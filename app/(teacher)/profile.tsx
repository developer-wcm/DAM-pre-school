...
export default function TeacherProfileScreen() {
  ...
  const initials = profile?.full_name
    ? profile.full_name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2) : 'TC';
    ...
    <LinearGradient colors={['#EDE9F6', '#F0EEF8', '#EAF0F8']} style={styles.container}>
      <View style={styles.profileCard}>
        ...
        <View style={styles.roleBadge}>
          <Ionicons name="school" size={13} color="#2A9D6E" />
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 72, height: 72, borderRadius: 36, backgroundColor: '#C8EDE3', justifyContent: 'center', alignItems: 'center' },
  avatarText: { fontSize: 26, fontWeight: '800', color: '#2A9D6E' },
  name: { fontSize: 20, fontWeight: '800', color: '#1A1A2E' },
  roleBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#D4F4E8', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5 },
  roleText: { fontSize: 11, fontWeight: '700', color: '#2A9D6E', letterSpacing: 1 },
